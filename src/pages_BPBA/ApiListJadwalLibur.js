...
class GetJadwalLibur extends Component {
    constructor(props) {
    super(props);
    this.state = {
        items: [],
   ...
   /* SAMPLE UNTUK POST DATA */
    componentDidMount() {
        ...
        fetch("http://18.191.9.5:8090/day-off/list")
            .then(res => res.json())
            .then(parsedJSON =>
            parsedJSON.dayoff.map(data => ({
                getId: `${data.Id}`,
                getTanggal: `${new Date(data.tanggal).toUTCString()}`,
        ...
        <Table striped bordered hover responsive>
            ...
            <tbody style={{textAlign:'center'}}>
                {items.length > 0 ? items.map(item => {
                    const { getId, getTanggal, getDesc } = item;
                    return(
                        <tr key={getId}>
                            <td>{getId}</td>
                            <td>{getTanggal}</td>
